...
static PROMPTS_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/src/builder/prompts");
...
impl Default for BuilderRouter {
    fn default() -> Self {
        Self::new()
    }
}
...
impl BuilderRouter {
    pub fn new() -> Self {
        ...
        let base_instructions = match os {
            "windows" => formatdoc! {r#"
                The builder extension gives you the capabilities to edit code files and run shell commands,
            ...
            _ => formatdoc! {r#"
                The builder extension gives you the capabilities to edit code files and run shell commands,
                and can be used to solve a wide range of problems. Expect the project to be structured as
                a collection of behaviors (features) that are described in plan.yaml. The first thing
                you should do is read and understand this file.

               **Important** when you are asked to create or modify functionality, you MUST update the plan
               file first by either adding a behavior or modifying an existing behavior as appropriate. Any
               files you touch as part of programming these behaviors should be linked in the plan.yaml file
               under the 'files' key of the relevant behavior. To understand a behavior before modifying it,
               start with the 'files' linked in the behavior.

               As you make changes, ensure you minimize dependencies
               between behaviors and use shared core types and interfaces as much as possible to reduce
               interdependent code.

               **Important** files must NOT exceed 500 lines of code. If they do, refactor for better modularization.
               **Important** always quote the id of the relevant behavior when making any changes at all.
        ...
        if global_hints_path.is_file() {
            if let Ok(global_hints) = std::fs::read_to_string(&global_hints_path) {
                hints.push_str("\n### Global Hints\nThe builder extension includes some global hints that apply to all projects & directories.\n");
        ...
        if local_hints_path.is_file() {
            if let Ok(local_hints) = std::fs::read_to_string(&local_hints_path) {
                ...
                hints.push_str("### Project Hints\nThe builder extension includes some hints for working on the project in this directory.\n");
...
mod tests {
    ...
    static DEV_ROUTER: OnceCell<BuilderRouter> = OnceCell::const_new();

    async fn get_router() -> &'static BuilderRouter {
        DEV_ROUTER
            .get_or_init(|| async { BuilderRouter::new() })
            .await
    }
    ...
    async fn test_goose_ignore_basic_patterns() {
        ...
        // Create a BuilderRouter with custom ignore patterns
    ...
    async fn test_text_editor_respects_ignore_patterns() {
        ...
        let router = BuilderRouter {
            tools: BuilderRouter::new().tools, // Reuse default tools
            prompts: Arc::new(HashMap::new()),
            instructions: String::new(),
            file_history: Arc::new(Mutex::new(HashMap::new())),
            ignore_patterns: Arc::new(ignore_patterns),
